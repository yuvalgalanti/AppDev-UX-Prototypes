...
>): TURLParamStateTuple<TDeserializedParams> => {
  ...
  const setParams = (newParams: Partial<TDeserializedParams>) => {
    ...
    // we use document.location.search here as the current params so these calls never overwrite
    // each other. This also retains any unrelated params that might be present and allows
    // newParams to be a partial update.
    //
    // For `pathname` we deliberately use `history.location.pathname` instead of
    // `document.location.pathname`: React Router's `history` is basename-relative and
    // `history.replace()` re-applies the router's `basename` itself, so passing the raw,
    // already basename-prefixed `document.location.pathname` here would duplicate the
    // basename on every call (compounding into a runaway redirect loop when this app is
    // hosted under a subpath, e.g. GitHub Pages).
