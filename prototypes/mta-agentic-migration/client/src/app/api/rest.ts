...
export { template };
// Scope the mocked `/hub` API path under the app's public path so MSW's
// service-worker-scoped intercepts work when hosted at a GitHub Pages subpath.
//
// NOTE: this must be computed *inside* the function body, not hoisted into a
// module-level `const`. `export * from "./rest/*"` below is hoisted (as a
// `require()`) above the rest of this module's body by the TS/webpack CJS
// interop, and some of those submodules call `hub` at their own top level
// (e.g. `export const AGENTS = hub\`/agents\`;`). If `hub` depended on a
// module-level `const` declared further down in this file, those early
// calls would hit it before it was initialized (TDZ `ReferenceError`).
// `PUBLIC_PATH` itself is safe to reference here because it comes from an
// earlier, independent import that has no dependency back on this module.
export function hub(tsa: TemplateStringsArray, ...vals: unknown[]): string {
  ...
  return `${PUBLIC_PATH.replace(/\/$/, "")}/hub${path}`;
}
