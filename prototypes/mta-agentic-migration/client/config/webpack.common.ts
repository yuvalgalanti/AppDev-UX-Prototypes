...
const config: Configuration = {
  ...
  plugins: [
    ...
    // Use a dedicated global identifier (not "process.env.*") for this
    // prototype-only build setting, so it doesn't interact with the blanket
    // "process.env": "{}" replacement above (two DefinePlugin instances both
    // rewriting `process.env.*` expressions has been observed to produce
    // broken/TDZ-erroring minified output).
    new DefinePlugin({
