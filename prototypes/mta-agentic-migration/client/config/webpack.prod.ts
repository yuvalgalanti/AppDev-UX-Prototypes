...
// This prototype is statically hosted (GitHub Pages) with no backend and no
// server-side `entrypoint.sh` to inject `window._env` at container start.
// Instead we bake a fixed, fully-mocked env into `index.html` at build time.
...
const config = merge<Configuration>(commonWebpackConfiguration, {
  ...
  plugins: [
    ...
    // Static prototype build: emit a real `index.html` (not `index.html.ejs`)
    // with `window._env` baked in at build time, since there is no
    // `entrypoint.sh`/express server to inject it at container start.
